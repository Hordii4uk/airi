...
Shopify.cartPopapForm = function(variant_id) {
  	var image = jQuery('#ProductPhotoImg').attr("src");
        ...
        if (key === 'items') {
          ...
          if (value.length == 1) {
            ...
            jQuery.each(value, function(i, item) {
              ...
              jQuery('<li class="mini-cart__product"><a href="javascript:void(0);" onclick="Shopify.removeItem(' + item.variant_id + ')" class="remove-from-cart remove"><i class="dl-icon-close"></i></a><div class="mini-cart__product__image"><img src="' + item.image + '" alt="{{ item.alt }}"></div><div class="mini-cart__product__content"><a class="mini-cart__product__title" href="' + item.url + '">' + item.title + '</a><span class="mini-cart__product__quantity">' + item.quantity + ' x ' + Shopify.formatMoney(item.price, formatMoney) + '</span></div></li>').appendTo(table);
            });
          }			
            ...
            jQuery(".single-product-cart").css({"display": "none"});
